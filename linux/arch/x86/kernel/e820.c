...
{
	printk("early boot in memory_setup\n");
	char *who = "BIOS-e820";
	u32 new_nr;
	/*
	...
	if (append_e820_map(boot_params.e820_map, boot_params.e820_entries)
	  < 0) {
		...
		/* Pip : we have memory available from 0x1202000 to 0x3FFEOOOO
		e820_add_region(HIGH_MEMORY, mem_size << 10, E820_RAM); */
		e820_add_region(0x0, 0x700000, E820_RESERVED); /* Hypervisor space */
		e820_add_region(0x700000, 0x1200000 - 0x700000, E820_RAM); /* Kernel area */
		e820_add_region(0x1202000, 0x10000000 - 0x1202000, E820_RAM); /* Free area */
		e820_add_region(0xFFFFF000, 0x1000, E820_RESERVED); /* VIDT */
	}
...
void __init memblock_x86_fill(void)
{
	...
	for (i = 0; i < e820->nr_map; i++) {
		struct e820entry *ei = &e820->map[i];
		printk("early console in memblock_x86_fill: entry at %p\n", ei);
		...
		printk("early console in memblock_x86_fill: requesting memblock add from %p size %x\n", ei->addr, ei->size);
		memblock_add(ei->addr, ei->size);
	}

	printk("throwing away partial pages\n");
	/* throw away partial pages */
	memblock_trim_memory(PAGE_SIZE);

	printk("dumping memblock\n");
	__memblock_dump_all();
}
